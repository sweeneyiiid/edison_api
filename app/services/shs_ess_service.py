...
DATABASE_NAME = 'shs_ess' #could also be table name

class ShsEssImport():
    """
    Class SHS ESS Service Steps
    ...
    def save_to_sql(self):
        """
        Call the SHS_wrapper
        Save data to sql
        """
        api_data = self.get_data()

        save_to_sql = ShsEssWrapper(api_data, self.token)
        res = save_to_sql.output()
